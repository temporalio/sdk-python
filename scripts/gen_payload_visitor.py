...
class PayloadVisitorGenerator:
    def __init__(self) -> None:
        ...
        self.runtime_type_imports: dict[str, tuple[str, str]] = {}
        self.runtime_module_imports: set[str] = set()
    ...
    def generate(self, roots: list[Descriptor]) -> str:
        ...
        extra_imports = "\n".join(
            ...
            for class_name, module in sorted(
                set(self.runtime_type_imports.values()),
                key=lambda item: (item[1], item[0]),
            )
        )
        module_imports = "\n".join(
            f"import {module}" for module in sorted(self.runtime_module_imports)
        )
        if extra_imports:
            extra_imports += "\n"
        if module_imports:
...
"""
    ...
    def python_type_for_descriptor(self, desc: Descriptor) -> str:
        ...
        self.runtime_module_imports.add(module)
