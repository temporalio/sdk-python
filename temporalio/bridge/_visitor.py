...
class PayloadVisitor:
    ...
    async def _visit_temporal_api_failure_v1_Failure(self, fs, o):
        ...
        if o.HasField("encoded_attributes"):
            coros.append(
                self._visit_temporal_api_common_v1_Payload(fs, o.encoded_attributes)
            )
        if o.HasField("cause"):
            coros.append(self._visit_temporal_api_failure_v1_Failure(fs, o.cause))
        if o.HasField("application_failure_info"):
            coros.append(
                self._visit_temporal_api_failure_v1_ApplicationFailureInfo(
                    fs, o.application_failure_info
                )
            )
        elif o.HasField("timeout_failure_info"):
            coros.append(
                self._visit_temporal_api_failure_v1_TimeoutFailureInfo(
                    fs, o.timeout_failure_info
                )
            )
        elif o.HasField("canceled_failure_info"):
            coros.append(
                self._visit_temporal_api_failure_v1_CanceledFailureInfo(
                    fs, o.canceled_failure_info
                )
            )
        elif o.HasField("reset_workflow_failure_info"):
            coros.append(
                self._visit_temporal_api_failure_v1_ResetWorkflowFailureInfo(
    ...
    async def _visit_coresdk_workflow_activation_InitializeWorkflow(self, fs, o):
        coros: list[Coroutine[Any, Any, None]] = []
        coros.append(self._visit_payload_container(fs, o.arguments))
        ...
        if o.HasField("continued_failure"):
            coros.append(
                self._visit_temporal_api_failure_v1_Failure(fs, o.continued_failure)
            )
        if o.HasField("last_completion_result"):
            coros.append(
                self._visit_temporal_api_common_v1_Payloads(
                    fs, o.last_completion_result
                )
            )
        if o.HasField("memo"):
            coros.append(self._visit_temporal_api_common_v1_Memo(fs, o.memo))
    ...
    async def _visit_coresdk_workflow_activation_SignalWorkflow(self, fs, o):
        coros: list[Coroutine[Any, Any, None]] = []
        coros.append(self._visit_payload_container(fs, o.input))
        if not self.skip_headers:
            coros.extend(
                self._visit_temporal_api_common_v1_Payload(fs, v)
                for v in o.headers.values()
    ...
    async def _visit_coresdk_workflow_activation_WorkflowActivation(self, fs, o):
        await asyncio.gather(
            *[
                self._visit_coresdk_workflow_activation_WorkflowActivationJob(fs, v)
                for v in o.jobs
            ]
        )
    ...
    async def _visit_temporal_api_sdk_v1_UserMetadata(self, fs, o):
        ...
        if o.HasField("summary"):
            coros.append(self._visit_temporal_api_common_v1_Payload(fs, o.summary))
        if o.HasField("details"):
            coros.append(self._visit_temporal_api_common_v1_Payload(fs, o.details))
        await asyncio.gather(*coros)
    ...
    async def _visit_coresdk_workflow_commands_SignalExternalWorkflowExecution(
        ...
        coros: list[Coroutine[Any, Any, None]] = []
        coros.append(self._visit_payload_container(fs, o.args))
        if not self.skip_headers:
            coros.extend(
                self._visit_temporal_api_common_v1_Payload(fs, v)
                for v in o.headers.values()
    ...
    async def _visit_coresdk_workflow_completion_Success(self, fs, o):
        await asyncio.gather(
            *[
                self._visit_coresdk_workflow_commands_WorkflowCommand(fs, v)
                for v in o.commands
            ]
        )
