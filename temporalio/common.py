...
class ValueHandle(Generic[temporalio.types.AnyType]):
    ...
    instead of the materialized value; callers are unaffected and may still pass a
    plain ``T``. Forward it onward without cost, and acquire its value where
    fetching data is permitted via :py:meth:`get_value`.
...
) -> MethodAsyncNoParam[ProtocolSelfType, ValueHandle[ProtocolReturnType]]: ...


@overload
def as_value_handle(
    fn: MethodSyncNoParam[ProtocolSelfType, ProtocolReturnType],
) -> MethodSyncNoParam[ProtocolSelfType, ValueHandle[ProtocolReturnType]]: ...
...
def as_value_handle(  # type: ignore[overload-overlap,reportOverlappingOverload]
    ...
    ProtocolSelfType,
    ProtocolParamType,
    ValueHandle[ProtocolReturnType],
]: ...
...
) -> CallableAsyncNoParam[ValueHandle[ProtocolReturnType]]: ...


@overload
def as_value_handle(
    fn: CallableSyncNoParam[ProtocolReturnType],
) -> CallableSyncNoParam[ValueHandle[ProtocolReturnType]]: ...


@overload
def as_value_handle(  # type: ignore[overload-overlap,reportOverlappingOverload]
    fn: CallableAsyncSingleParam[ProtocolParamType, ProtocolReturnType],
) -> CallableAsyncSingleParam[ProtocolParamType, ValueHandle[ProtocolReturnType]]: ...


@overload
def as_value_handle(
    fn: CallableSyncSingleParam[ProtocolParamType, ProtocolReturnType],
) -> CallableSyncSingleParam[ProtocolParamType, ValueHandle[ProtocolReturnType]]: ...
...
) -> Callable[..., Awaitable[ValueHandle[ProtocolReturnType]]]: ...


@overload
def as_value_handle(  # type: ignore[overload-cannot-match]
    fn: Callable[..., ProtocolReturnType],
) -> Callable[..., ValueHandle[ProtocolReturnType]]: ...


def as_value_handle(fn: Any) -> Any:
    """Declare that a call's result is consumed as a :py:class:`ValueHandle`.
    ...
    Usable from any context that starts something with a result: workflow code
    declaring an activity or child-workflow result, and client code declaring a
    workflow result. For a dynamic (string-named) target there is no callable to
