...
wrapper. Instead, `CloudRunPlugin` reads Cloud Run instance metadata and configures a normal,
...
When the client connects, `CloudRunPlugin` resolves the deployment name from `CLOUD_RUN_WORKER_POOL`
...
```python
from temporalio.contrib.gcp.cloud_run import CloudRunPlugin, get_google_cloud_run_metadata

metadata = get_google_cloud_run_metadata()
plugin = CloudRunPlugin(metadata=metadata)
