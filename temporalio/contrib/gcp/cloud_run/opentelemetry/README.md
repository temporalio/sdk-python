...
Install the SDK with the Cloud Run OpenTelemetry dependencies:

```bash
python -m pip install 'temporalio[cloud-run-worker-otel]'
```
...
`temporalio.contrib.opentelemetry.OpenTelemetryPlugin`. Do not install both the
Cloud Run and generic OpenTelemetry plugins on the same client.
...
OpenTelemetry has one global tracer provider per process. Create the Cloud Run
plugin before installing another provider, or pass the already-installed
replay-safe provider through `tracer_provider=`. The plugin raises instead of
silently using a different global provider.
