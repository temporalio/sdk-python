...
from temporalio.contrib.opentelemetry import _serverless, create_tracer_provider
...
OTEL_EXPORTER_OTLP_ENDPOINT_ENV_VAR = _serverless.OTEL_EXPORTER_OTLP_ENDPOINT_ENV_VAR
"""Standard OpenTelemetry environment variable for the common OTLP endpoint."""

OTEL_SERVICE_NAME_ENV_VAR = _serverless.OTEL_SERVICE_NAME_ENV_VAR
...
DEFAULT_OTLP_ENDPOINT = _serverless.DEFAULT_OTLP_ENDPOINT
    ...
    def __init__(
        ...
        *,
        ...
        try:
            if runtime is None:
                runtime = Runtime(
                    telemetry=_serverless.build_metrics_telemetry_config(
...
def _create_tracer_provider(
    ...
    try:
        provider.add_span_processor(_serverless.build_otlp_span_processor(endpoint))
    except ImportError as err:
        raise RuntimeError(
            "The OTLP gRPC exporter is required. Install the "
            "'temporalio[cloud-run-worker-otel]' extra."
        ) from err
    return provider
...
def _resolve_endpoint(explicit: str | None) -> str:
    return _serverless.resolve_endpoint(explicit)


def _resolve_service_name(explicit: str | None) -> str:
    return _serverless.resolve_service_name(
        explicit,
        [CLOUD_RUN_WORKER_POOL_ENV_VAR, CLOUD_RUN_SERVICE_ENV_VAR],
