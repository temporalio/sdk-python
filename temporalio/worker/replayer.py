            ...
            async for result in replay_iterator:
                if result.replay_failure:
                    if raise_on_replay_failure:
                        raise result.replay_failure
                    replay_failures[result.run_id] = result.replay_failure
            return WorkflowReplayResults(replay_failures=replay_failures)

    @asynccontextmanager
    async def workflow_replay_iterator(self, histories: AsyncIterator[temporalio.client.WorkflowHistory]) -> AsyncIterator[AsyncIterator[WorkflowReplayResult]]:
        ...
        try:
            ...
            def on_eviction_hook(run_id: str, remove_job: temporalio.bridge.proto.workflow_activation.RemoveFromCache) -> None:
                nonlocal last_replay_failure, last_replay_run_id
                last_replay_run_id = run_id
                if remove_job.reason == temporalio.bridge.proto.workflow_activation.RemoveFromCache.EvictionReason.NONDETERMINISM:
                    last_replay_failure = temporalio.workflow.NondeterminismError(remove_job.message)
                elif remove_job.reason != temporalio.bridge.proto.workflow_activation.RemoveFromCache.EvictionReason.CACHE_FULL and remove_job.reason != temporalio.bridge.proto.workflow_activation.RemoveFromCache.EvictionReason.LANG_REQUESTED:
                    last_replay_failure = RuntimeError(f"{remove_job.reason}: {remove_job.message}")
                else:
                    last_replay_failure = None
                last_replay_complete.set()

            # Start the worker
            workflow_worker_task = asyncio.create_task(_WorkflowWorker(
            ...
            async def replay_iterator() -> AsyncIterator[WorkflowReplayResult]:
                async for history in histories:
                    ...
                    await pusher.push_history(history.workflow_id, temporalio.api.history.v1.History(events=history.events).SerializeToString())
                    ...
                    # is enabled.
                    await asyncio.wait(
                        [workflow_worker_task, asyncio.create_task(last_replay_complete.wait())],
                    ...
                    yield WorkflowReplayResult(
                        run_id=last_replay_run_id,
                        history=history,
                        replay_failure=last_replay_failure,
                    )
            yield replay_iterator()

            # Wait for worker to complete normally
            await workflow_worker_task
        finally:
            # Close the pusher
            pusher.close()
...
class WorkflowReplayResult:
    # Technically available via history
    run_id: str
    history: temporalio.client.WorkflowHistory
