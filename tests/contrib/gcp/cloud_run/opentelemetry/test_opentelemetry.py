"""Tests for the Google Cloud Run OpenTelemetry plugin."""
...
def test_ignores_empty_environment_values(
    ...
    # Empty environment values are falsy and are skipped, falling through to
    # the next source in the chain. Whitespace is not stripped, so only truly
    # empty values are ignored (matching the shared serverless behavior).
