...
from temporalio.workflow._event_groups import (
    _active_event_groups,
    _ActiveEventGroups,
    _capture_event_group_markers,
    _inbound_event_group,
    _LabelEventGroup,
    _StubImplicitEventGroup,
)
...
def test_invalid_inbound_event_id_is_noop_stub() -> None:
    ...
    enclosing = _ActiveEventGroups(explicit={"id": _LabelEventGroup("id", "label")})
    applied = stub._applied_over(enclosing)
    assert applied.implicit is None
    assert applied.explicit == {}

    token = _active_event_groups.set(applied)
    try:
        assert _capture_event_group_markers(None) == []
    finally:
...
@workflow.defn
class ExternalWorkflowCommandsWorkflow:
    @workflow.run
    async def run(self) -> None:
        direct = workflow.create_event_group("direct")
        scope = workflow.create_event_group("scope")
        missing = workflow.get_external_workflow_handle("event-groups-no-such-workflow")
        with scope.scope():
            try:
                await missing.signal("signal", event_groups=[direct])
            except ApplicationError:
                pass
            try:
                await missing.cancel(event_groups=[direct])
            except ApplicationError:
                pass


async def test_external_workflow_commands_carry_markers(
    client: Client, env: WorkflowEnvironment
):
    _require_event_groups_server(env)

    async with new_worker(client, ExternalWorkflowCommandsWorkflow) as worker:
        handle = await client.start_workflow(
            ExternalWorkflowCommandsWorkflow.run,
            id=f"workflow-{uuid.uuid4()}",
            task_queue=worker.task_queue,
        )
        await handle.result()
...
@workflow.defn
class MetadataCommandsWorkflow:
    @workflow.run
    async def run(self) -> None:
        direct = workflow.create_event_group("direct")
        scope = workflow.create_event_group("scope")
        with scope.scope():
            workflow.upsert_memo({"some-key": "some-value"}, event_groups=[direct])
            workflow.upsert_search_attributes(
                [SearchAttributeKey.for_bool("CustomBoolField").value_set(False)],
                event_groups=[direct],
            )
            workflow.patched("my-patch-1", event_groups=[direct])
            workflow.deprecate_patch("my-patch-2", event_groups=[direct])


@pytest.mark.requires_local_server
async def test_metadata_commands_carry_markers(
    client: Client, env: WorkflowEnvironment
):
    _require_event_groups_server(env)
    ...
    async with new_worker(client, MetadataCommandsWorkflow) as worker:
        handle = await client.start_workflow(
            MetadataCommandsWorkflow.run,
            id=f"workflow-{uuid.uuid4()}",
            task_queue=worker.task_queue,
        )
        await handle.result()
