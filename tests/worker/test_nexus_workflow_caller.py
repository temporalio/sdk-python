...
class HandlerWorkflow:
    ...
    async def run(
        ...
        start_options_received_by_handler: nexusrpc.handler.StartOperationOptions,
    ) -> HandlerWfOutput:
...
class SyncOrAsyncOperation:
    ...
    # TODO(dan): get rid of these; should not be necessary; they are only being
    # implemented internally temporarily as a check of design.
...
@nexusrpc.handler.service(interface=ServiceInterface)
class ServiceImpl:
...
class ServiceInterfaceWithoutNameOverride:
    op: nexusrpc.interface.Operation[None, ServiceClassNameOutput]


@nexusrpc.interface.service(name="service-interface-🌈")
...
@nexusrpc.handler.service(interface=ServiceInterfaceWithoutNameOverride)
...
@nexusrpc.handler.service(interface=ServiceInterfaceWithNameOverride)
