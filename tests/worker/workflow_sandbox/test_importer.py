...
import pytest
from temporalio.worker.workflow_sandbox.importer import Importer, _RestrictedEnvironment
from temporalio.worker.workflow_sandbox.restrictions import RestrictedWorkflowAccessError
from .testmodules import restrictions

def test_workflow_sandbox_importer_invalid_module():
    importer = Importer(_RestrictedEnvironment(restrictions))
    ...
    assert err.value.qualified_name == "tests.worker.workflow_sandbox.testmodules.invalid_module"
...
def test_workflow_sandbox_importer_invalid_module_members():
    ...
    assert err.value.qualified_name == "tests.worker.workflow_sandbox.testmodules.invalid_module_members.invalid_function.__call__"

    # Cannot call via from import either
    with pytest.raises(RestrictedWorkflowAccessError) as err:
        with importer.applied():
            from tests.worker.workflow_sandbox.testmodules.invalid_module_members import invalid_function
            invalid_function()
    assert err.value.qualified_name == "tests.worker.workflow_sandbox.testmodules.invalid_module_members.invalid_function.__call__"
